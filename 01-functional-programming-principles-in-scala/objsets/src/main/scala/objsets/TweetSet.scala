...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  //My Own
  override def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = right.filterAcc(p,
  ...
  override def mostRetweetedAcc(acc: Tweet): Tweet = right.mostRetweetedAcc(left.mostRetweetedAcc(if (elem.retweets > acc.retweets) elem else acc))
