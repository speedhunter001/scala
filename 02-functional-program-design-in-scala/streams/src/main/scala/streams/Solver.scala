...
trait Solver extends GameDef {
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val visited = explored + block
      val allN = neighborsWithHistory(block, moves)
      val newN = newNeighborsOnly(allN, visited)
      val stream = initial.tail ++ newN

      if (stream.isEmpty) LazyList.empty
      else stream.head #:: from(stream, visited)
    }
  }
