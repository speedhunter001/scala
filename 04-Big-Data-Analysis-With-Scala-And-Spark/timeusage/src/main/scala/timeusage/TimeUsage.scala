...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val workingActivitiesCols: List[String] = List("t05", "t1805")
    val otherActivitiesCols: List[String] = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    (columnNames.filter(col => primaryNeedsCols.exists(c => col.startsWith(c))).map(c => col(c)),
      columnNames.filter(col => workingActivitiesCols.exists(c => col.startsWith(c))).map(c => col(c)),
      columnNames
        .filter(col => !col.startsWith("t1801") && !col.startsWith("t1803") && !col.startsWith("t1805") && otherActivitiesCols.exists(c => col.startsWith(c)))
        .map(c => col(c))
    )
  }
